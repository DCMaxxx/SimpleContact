...
- (void)setLeftViewInformationsWithContact:(SCContact *)contact {
    NSArray * availableKinds = [SCKindHandler enabledKinds];
    ...
    if (!_leftView) {
        ...
        for (NSUInteger i = 0; i < numberOfKinds; ++i) {
            ...
            eContactNumberKind kind = [SCKindHandler kindFromString:[availableKinds objectAtIndex:i]];

            UIImageView * imageView = [[UIImageView alloc] initWithFrame:frame];
            [imageView setTag:(kTagLowestValue - kind - 1)];
            [imageView setImage:[SCKindHandler iconForKind:kind andWhite:NO]];
...
/*----------------------------------------------------------------------------*/
- (void)setLeftViewInformationsWithContact:(SCContact *)contact kind:(eContactNumberKind)kind andImageView:(UIImageView *)imageView {
    NSString * const ImgUnavalaibleOverlay = @"unavailable-overlay.png";
    SEL selector = [SCKindHandler selectorForKind:kind prefix:@"tappedOn" andSuffix:@":"];
