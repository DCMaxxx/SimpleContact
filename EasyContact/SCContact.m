...
- (NSString *)importantName {
    if ([[SCSettingsHandler sharedInstance] getOption:eSOFirstName ofCategory:eSCListOrder])
...
- (NSString *)favoriteName {
    if ([[SCSettingsHandler sharedInstance] getOption:eSOFirstName ofCategory:eSCFavoriteOrder])
        return [self firstName];
    else if ([[SCSettingsHandler sharedInstance] getOption:eSOLastName ofCategory:eSCFavoriteOrder])
        return [self lastName];
...
- (NSInteger)numberOf:(eContactNumberKind)kind {
    SEL selector = [SCKindHandler selectorForKind:kind prefix:@"numberOf" andSuffix:@"s"];
...
- (NSArray *)addessesOf:(eContactNumberKind)kind {
    SEL selector = [SCKindHandler selectorForKind:kind prefix:@"addressesFor" andSuffix:nil];
    if ([self respondsToSelector:selector]) {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
...
- (NSArray *)addressesForPhone {
    NSMutableArray * result = [_addresses objectForKey:[SCKindHandler kindToString:eCNKPhone]];
    if (!result) {
        ...
        for (CFIndex j = 0; j < ABMultiValueGetCount(phones); j++) {
            ...
            if ([[SCFavoritesHandler sharedInstance] isFavoriteWithContact:self number:phoneNumber ofKind:eCNKPhone])
                [tmp setObject:@(YES) forKey:DicKeyFavorite];
            [result addObject:tmp];
        }
        [_addresses setObject:result forKey:[SCKindHandler kindToString:eCNKPhone]];
    }
    return result;
}

- (NSArray *)addressesForMail {
    NSMutableArray * result = [_addresses objectForKey:[SCKindHandler kindToString:eCNKMail]];
    if (!result) {
        result = [[NSMutableArray alloc] init];
        ...
        for (CFIndex j = 0; j < ABMultiValueGetCount(mails); j++) {
            ...
            if ([[SCFavoritesHandler sharedInstance] isFavoriteWithContact:self number:mailAddress ofKind:eCNKMail])
                [tmp setObject:@(YES) forKey:DicKeyFavorite];
            [result addObject:tmp];
        }
        [_addresses setObject:result forKey:[SCKindHandler kindToString:eCNKMail]];
    }
    return result;
...
- (NSArray *)addressesForText {
    NSMutableArray * result = [_addresses objectForKey:[SCKindHandler kindToString:eCNKText]];
    if (!result) {
        ...
        for (NSMutableDictionary * number in result) {
            if ([[SCFavoritesHandler sharedInstance] isFavoriteWithContact:self number:[number objectForKey:DicKeyValue] ofKind:eCNKText])
                [number setObject:@(YES) forKey:DicKeyFavorite];
        }
        [_addresses setObject:result forKey:[SCKindHandler kindToString:eCNKText]];
    }
    return result;
}

- (NSArray *)addressesForFaceTime {
    NSMutableArray * result = [_addresses objectForKey:[SCKindHandler kindToString:eCNKFaceTime]];
    if (!result) {
        result = [self deepCopy:[self addressesForText]];
        for (NSMutableDictionary * number in result) {
            if ([[SCFavoritesHandler sharedInstance] isFavoriteWithContact:self number:[number objectForKey:DicKeyValue] ofKind:eCNKFaceTime])
                [number setObject:@(YES) forKey:DicKeyFavorite];
        }
        [_addresses setObject:result forKey:[SCKindHandler kindToString:eCNKFaceTime]];
    }
    return result;
}
